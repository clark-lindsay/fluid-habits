defmodule FluidHabitsWeb.CoreComponents do
  @moduledoc """
        ...
        <div class="flex min-h-full items-center justify-center">
          <div class="w-full max-w-3xl p-4 sm:p-6 lg:py-8">
            <.focus_wrap
              ...
              class="hidden relative rounded-2xl bg-white dark:bg-gray-800 p-14 shadow-lg shadow-zinc-700/10 ring-1 ring-zinc-700/10 transition"
            >
              <div class="absolute top-6 right-5">
                ...
                >
                  <Heroicons.x_mark outline class="h-6 w-6 stroke-current dark:stroke-gray-50" />
                </button>
              </div>
              <div id={"#{@id}-content"}>
                <header :if={@title != []}>
                  ...
                  <p
                    :if={@subtitle != []}
                    id={"#{@id}-description"}
                    class="mt-2 text-sm leading-6 text-zinc-600 dark:text-gray-200"
                  >
  ...
  def simple_form(assigns) do
    ...
    <.form :let={f} for={@for} as={@as} {@rest}>
      <div class="space-y-8 bg-white dark:bg-gray-800">
  ...
  def input(%{type: "select"} = assigns) do
    ...
    <div phx-feedback-for={@name}>
      ...
      <select
        id={@id}
        name={@name}
        class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white dark:bg-gray-600 dark:text-gray-100 rounded-md shadow-sm focus:outline-none focus:ring-primary-700 focus:border-primary-700 sm:text-sm"
        multiple={@multiple}
        {@rest}
      >
    ...
    <div phx-feedback-for={@name}>
      ...
      <textarea
        ...
        class={[
          ...
          "text-zinc-900 dark:bg-gray-600 dark:text-gray-100 focus:border-zinc-400 focus:outline-none focus:ring-4 focus:ring-zinc-800/5 sm:text-sm sm:leading-6",
    ...
    <div phx-feedback-for={@name}>
      ...
      <input
        ...
        class={[
          ...
          "text-zinc-900 dark:bg-gray-600 dark:text-gray-100 focus:outline-none focus:ring-4 sm:text-sm sm:leading-6",
          "phx-no-feedback:border-zinc-300 phx-no-feedback:focus:border-zinc-400 phx-no-feedback:focus:ring-zinc-800/5",
          "border-zinc-300 focus:border-zinc-400 focus:ring-zinc-800/5",
          @errors != [] && "border-rose-400 focus:border-rose-400 focus:ring-rose-400/10"
        ]}
  ...
  def label(assigns) do
    ~H"""
    <label for={@for} class="block text-sm font-semibold leading-6 text-zinc-800 dark:text-gray-100">
      <%= render_slot(@inner_block) %>
    </label>
    """
  end
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(FluidHabitsWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(FluidHabitsWeb.Gettext, "errors", msg, opts)
    end
  end
